...
ipcMain.handle("command", (event, commandArguments: string): void => {
  const handlers: FFmpegCommandHandlers = {
    ...
    handleStart: (start): void => {
      mainWindow.webContents.send("ffmpeg-start", start);
    },
    handleEnd: (end): void => {
      mainWindow.webContents.send("ffmpeg-end", end);
    },
    handleData: (data): void => {
      mainWindow.webContents.send("ffmpeg-data", data);
    },
    handleProgress: (progress): void => {
      mainWindow.webContents.send("ffmpeg-progress", progress);
    },
    handleCodecData: (codecData): void => {
      mainWindow.webContents.send("ffmpeg-codecData", codecData);
    },
  };
